...
func main() {
	...
	if !remoteConfig {
		...
		ch, err = discovery.LoadDefaultRemoteConfig(remoteConfigAddr, discovery.ServiceConfigPrefix+"storage", remoteConfigToken, config.Conf)
		if err != nil {
			panic(err)
		}
	}
...
func startGRPCServer() {
	...
	svc = service.NewService(infra.FR, config.Conf)
