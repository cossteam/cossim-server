...
import (
	...
	"github.com/cossim/coss-server/interface/gateway/server/http"
...
func main() {
	...
	if !remoteConfig {
		...
		ch, err = discovery.LoadDefaultRemoteConfig(remoteConfigAddr, discovery.InterfaceConfigPrefix+"gateway", remoteConfigToken, config.Conf)
		if err != nil {
			panic(err)
		}
	}
